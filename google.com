...
<head>
    ...
    <style>
        /* ÇÓÊÝÇÏå ÇÒ ÝæäÊ ÇÓÊÇäÏÇÑÏ ÈÑÇí ÔÈíåÓÇÒí ÙÇåÑ æá */
        ...
        /* ÇÓÊÇíá ÈÑÇí äæÇÑ íãÇíÔ ÈÇáÇ ˜å ÍÇáÊ ÌÓÊÌæ íÇ ÇÎÈÇÑ ÑÇ äÔÇä ãíÏåÏ */
        .tab-button {
            /* ÊäÙíãÇÊ ÇÕáí: Ïíä¡ ã˜ÇääãÇ¡ ÇäÏÇÒå ÝæäÊ¡ ÎØ ÒíÑíä ÔÝÇÝ¡ Ñä ãÊä íÔÝÑÖ ÎÇ˜ÓÊÑí */
            @apply p-3 cursor-pointer text-sm font-medium border-b-4 border-transparent text-gray-500 transition-colors duration-150;
        }

        /* ÍÇáÊ åÇæÑ ÈÑÇí ÊãÇã Ï˜ãååÇ: äãÇíÔ ÎØ ÎÇ˜ÓÊÑí ãáÇíã */
        .tab-button:hover {
            /* Çíä ÎØ ÒíÑíä ãáÇíã ÝÞØ åäÇã åÇæÑ ÙÇåÑ ãíÔæÏ */
            border-bottom-color: #e0e0e0; 
        }
        
        /* ÍÇáÊ ÝÚÇá: äãÇíÔ ÎØ ÂÈí/äíáí ÑÑä æ ÊÛííÑ Ñä ãÊä (ÇæáæíÊ ÈÇáÇÊÑ ÈÇ !important) */
        .tab-active {
            /* åäÇã ÝÚÇá ÈæÏä¡ ÎØ ÂÈí ÑÑä æ Ñä ãÊä ÂÈí ÑÑä ãíÔæÏ */
            @apply text-indigo-700 border-indigo-700 !important;
        }

        /* ÇÓÊÇíá ÈÑÇí ãÍá ãÎÝí äãÇíÔ ÑãÒ */
        .hidden-result {
            color: #dadce0; 
            font-size: 0.7rem;
            direction: ltr;
        ...
        /* ÇäíãíÔä ãÍæ ÔÏä ÈÑÇí äÊíÌå ÑãÒÔÇíí - 0.5 ËÇäíå */
        .fade-out {
            opacity: 0;
        ...
        /* ÍÐÝ ÎØ íÔÝÑÖ ÎÇ˜ÓÊÑí ÇÒ ÒíÑ åÏÑ ˜å ÈÇÚË ÇíÌÇÏ ÎØ äÇÎæÇÓÊå ãíÔÏ */
        .main-header {
            @apply w-full border-b-0; /* ÎØ ÒíÑíä åÏÑ ÇÕáí ÑÇ Ç˜ ãí˜äíã */
        }
        
        /* ÎØ ÎÇ˜ÓÊÑí ãáÇíã ÝÞØ Èå Çííä äæÇÑ äÇæÈÑí (nav) ÇÖÇÝå ãíÔæÏ ÊÇ ÊÈåÇ ÑÇ ÇÒ ãÍÊæÇí ÒíÑíä ÌÏÇ ˜äÏ */
        .nav-bar {
...
<body class="min-h-screen flex flex-col items-center">
    ...
    <header class="main-header w-full">
        ...
        <nav class="nav-bar max-w-6xl mx-auto px-4">
            <div class="flex space-x-6 space-x-reverse">
                <button id="search-tab" class="tab-button tab-active">ÌÓÊÌæ</button>
                <button id="news-tab" class="tab-button">ÇÎÈÇÑ</button>
                <button id="images-tab" class="tab-button">ÊÕÇæíÑ</button>
                <button id="maps-tab" class="tab-button">äÞÔååÇ</button>
            </div>
        </nav>
    </header>
    ...
    <main class="flex-1 w-full max-w-xl px-4 pt-10">
        ...
        <div id="mode-status" class="text-center mb-6 text-lg font-semibold text-gray-600">
            <!-- Çíä ÞÓãÊ åãíÔå ÎÇáí ÇÓÊ ÊÇ åí ãÊäí ÙÇåÑ äÔæÏ -->
        </div>

        <!-- Decryption Result Display (Hidden initially, appears on News mode success/failure) -->
        <!-- ˜ÇÏÑ ÈÏæä åí ÇÓÊÇíá åÔÏÇÑí¡ ÝÞØ í˜ Ïíæ äåÇä ÇÓÊ -->
        <div id="decryption-box" class="hidden p-3 text-center" role="alert">
            <!-- Çíä ÇÑÇÑÇÝ ÈÑÇí ÚäæÇä ÈæÏ ˜å ÍÇáÇ ÎÇáí ãíãÇäÏ -->
            <p class="font-bold mb-2"></p>
            <!-- ãÊä ÑãÒÔÇíí ÔÏå ÈÇ ÇäÏÇÒå æ Ñä ÚÇÏí -->
    ...
    <footer class="w-full mt-auto py-3 border-t border-gray-200 text-center">
        <p class="text-gray-500 text-xs">
            © 2025 Google - <a href="#" class="underline">ÍÑíã ÎÕæÕí</a> - <a href="#" class="underline">ÔÑÇíØ</a>
        </p>
    </footer>
    ...
    <script type="module">
        ...
        function base64Decode(str) {
            try {
                ...
                return "ÎØÇ ÏÑ ÑãÒÔÇíí Base64. (Error: Invalid Cipher Format)";
            }
        }
        ...
        async function setupFirebase() {
            try {
                ...
                console.error("ÎØÇ ÏÑ ÑÇåÇäÏÇÒí ÝÇíÑÈíÓ (Auth/DB):", error);
                // ÊäåÇ íÇã ÎØÇí ÓíÓÊãí ÏÑ ÇíäÌÇ ãÌÇÒ ÇÓÊ.
                displayError("ÎØÇ ÏÑ ÇÊÕÇá Èå ÓÑæíÓ. áØÝÇð ãÑæÑÑ ÑÇ ÑÝÑÔ ˜äíÏ.");
            }
        }
        
        // --- ãäØÞ ÊÛííÑ ÍÇáÊ (Mode Switching) ---

        function switchMode(mode) {
            currentMode = mode;
            
            // Ç˜ÓÇÒí ÍÇáÊ ÝÚÇá ÇÒ åãå ÊÈåÇ
            ...
            modeStatus.innerHTML = ''; // ÇØãíäÇä ÇÒ ÎÇáí ÈæÏä æÖÚíÊ

            // ÊäÙíã ÍÇáÊ ÝÚÇá ÈÑ ÇÓÇÓ ˜áí˜
        ...
        // --- ãäØÞ ãÔÊÑ˜ ÈÑÇí ÑÏÇÒÔ æÑæÏí ---
        function handleInput(e) {
            decryptionBox.classList.add('hidden');
            resultsArea.innerHTML = ''; // Ç˜ÓÇÒí äÊÇíÌ ÞÈáí
            modeStatus.innerHTML = ''; // ÍÐÝ åÑæäå íÇã æÖÚíÊ
            
            const query = searchInput.value;

            // 1. ãäØÞ ÑãÒÐÇÑí (ÝÞØ ÈÇ Enter ÏÑ ÍÇáÊ Encode)
            if (currentMode === 'encode' && e.type === 'keydown' && e.key === 'Enter') {
                ...
                // äãÇíÔ ÝÞØ áæÏÑ (Spinner) ÈÑÇí ÔÈíåÓÇÒí ÌÓÊÌæ
                resultsArea.innerHTML = `
            ...
            // 2. ãäØÞ ÑãÒÔÇíí (ÈáÇÝÇÕáå ÈÇ Input ÏÑ ÍÇáÊ Decode)
            ...
            // 3. ÇÑ ÍÇáÊ ÊÕÇæíÑ íÇ äÞÔååÇ ÝÚÇá ÈÇÔÏ¡ åí ˜ÇÑí ÇäÌÇã äãíÏåíã ÊÇ ØÈíÚí Èå äÙÑ ÈÑÓÏ.
        }
        ...
        async function encryptAndSave(plaintext) {
            if (!isAuthReady || !db) {
                displayError("ÎØÇ: ÇÍÑÇÒ åæíÊ íÇ ÇíÇå ÏÇÏå ÂãÇÏå äíÓÊ.");
                return;
            }
            ...
            try {
                ...
                // äãÇíÔ ÎØÇí ÌÚáí ãÑæÑÑ ÏÑ äÇÍíå äÊÇíÌ
                showFakeBrowserError(cipherId);

            } catch (error) {
                console.error("ÎØÇ ÏÑ ÑãÒÐÇÑí æ ÐÎíÑå:", error);
                displayError("ÎØÇ ÏÑ ÐÎíÑåÓÇÒí ÑãÒ ÏÑ ÇíÇå ÏÇÏå.");
            }
        }
        ...
        async function decryptAndFetch(stealthKeyInput) {
            ...
            if (!cipherId) {
                resultsArea.innerHTML = '';
                // ÏÑ ÕæÑÊ ÎØÇ¡ íÇã ÎØÇí äÇãÍÓæÓ ÑÇ ÏÑ ˜ÇÏÑ äãÇíÔ ãíÏåíã.
                displayDecryptedResult("ÎØÇ ÏÑ ÈÇÑÐÇÑí ãÍÊæÇ. ãã˜ä ÇÓÊ ÇÊÕÇá ÖÚíÝ ÈÇÔÏ.", true);
                return;
            }

            // áæÏÑ ÑÇ ÏÑ äÇÍíå äÊÇíÌ äãÇíÔ ãíÏåíã (˜ÇãáÇð ÔÈíå Èå ÌÓÊÌæí æÇÞÚí)
            resultsArea.innerHTML = `
                <div class="text-center mt-10">
                    <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-red-600 mx-auto mb-4"></div>
                    <p class="text-gray-500">ÏÑ ÍÇá ÈÇÒíÇÈí ÇØáÇÚÇÊ... (ID: ${cipherId})...</p>
                </div>
            `;
            ...
            try {
                ...
                if (cipherText) {
                    ...
                    searchInput.value = ''; 
                    // ÏÑ ÕæÑÊ ãæÝÞíÊ¡ ÝÞØ ˜ÇÏÑ ÑÇ äãÇíÔ ãíÏåíã.
                    displayDecryptedResult(decryptedTextResult, false); 
                } else {
                    // íÇã ÎØÇí äÇãÍÓæÓ ÏÑ ÕæÑÊ ÚÏã æÌæÏ ˜áíÏ ÏÑ ÏíÊÇÈíÓ
                    displayDecryptedResult(`ÎØÇ ÏÑ ÈÇÒíÇÈí ÇØáÇÚÇÊ. ÓÑæÑ ÏÑ ÏÓÊÑÓ äíÓÊ.`, true);
                }
            } catch (error) {
                 console.error("ÎØÇ ÏÑ ÈÇÒíÇÈí æ ÑãÒÔÇíí:", error);
                 displayDecryptedResult("ÎØÇí ÛíÑãäÊÙÑå ÏÑ ÇÑÊÈÇØ ÈÇ ÇíÇå ÏÇÏå.", true);
            }
        }
        ...
        function displayError(message) {
            resultsArea.innerHTML = `
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg" role="alert">
                    <p class="font-bold mb-1">ÎØÇ:</p>
                    <p>${message}</p>
                </div>
            `;
        }
        
        // --- ÊæÇÈÚ ÌÏíÏ ÈÑÇí ÎØÇí ÌÚáí ãÑæÑÑ ÏÑ ÕÝÍå ---
        ...
        function showFakeBrowserError(cipherId) {
            ...
            // ÇÝÒæÏä ÔäæäÏå Èå Ï˜ãå Retry ÌÏíÏ
            ...
            retryButton.addEventListener('click', () => {
                if (copyToClipboard(stealthKey)) {
                    // --- ÊÛííÑÇÊ ÌÏíÏ: ÍÐÝ íÇã ãÊäí æ ÝÞØ ÊÛííÑ ÈÕÑí ---
                    statusMessage.innerHTML = ''; // íÇã æÖÚíÊ ÑÇ ÎÇáí äå ãíÏÇÑíã
                    retryButton.classList.remove('bg-blue-600', 'bg-red-600');
                    retryButton.classList.add('bg-green-600');
                    // *** ÎØí ˜å ãÊä Ï˜ãå ÑÇ Èå 'Copied' ÊÛííÑ ãíÏÇÏ ÍÐÝ ÔÏ. ***

                    // Ç˜ ˜ÑÏä íÇã Ó ÇÒ 3 ËÇäíå ÈÑÇí ÈÇÒÔÊ Èå ÍÇáÊ Çæáíå
                    setTimeout(() => {
                        resultsArea.innerHTML = '';
                        // ÇØãíäÇä ÇÒ ÈÇÒÔÊ Ï˜ãå Èå Ñä ÂÈí ÇÕáí (Ñå ÈÇ ÍÐÝ resultsArea ÏíÑ ãåã äíÓÊ)
                        // ÇãÇ ÈÑÇí ÊãíÒí ˜Ï:
                    ...
                    // ÏÑ ÕæÑÊ ÎØÇí ˜í¡ íÇã ÎØÇ ÑÇ äãÇíÔ ãíÏåíã.
                    ...
                    retryButton.textContent = originalButtonText; // ÇØãíäÇä ÇÒ ÍÝÙ ãÊä ÇÕáí
                }
            });
        }
        ...
        function displayDecryptedResult(textResult, isError) {
            ...
            // ÏÑ ÍÇáÊ ãæÝÞíÊ (Success)
            if (!isError) {
                // ãÊä ÑãÒÔÇíí ÔÏå ÈÇ Ñä ÚÇÏí (ÎÇ˜ÓÊÑí ÊíÑå) äãÇíÔ ÏÇÏå ãíÔæÏ
            ...
            // ÏÑ ÍÇáÊ ÎØÇ (Error)
                 ...
                 decryptedText.classList.add('text-red-500'); // ÈÑÇí äÔÇä ÏÇÏä ÎØÇ
                 titleElement.textContent = "";
            }

            // ãäØÞ 4 ËÇäíåÇí: 3.5 ËÇäíå äãÇíÔ + 0.5 ËÇäíå ãÍæ ÔÏä
            setTimeout(() => { 
                // 1. ÔÑæÚ ãÍæ ÔÏä
                decryptionBox.classList.add('fade-out'); 
                
                // 2. äåÇä ˜ÑÏä ˜Çãá ÈÚÏ ÇÒ ÇÊãÇã ÇäíãíÔä ãÍæ ÔÏä (0.5 ËÇäíå)
                setTimeout(() => {
                    decryptionBox.classList.add('hidden');
                    decryptionBox.classList.remove('fade-out');
                    modeStatus.innerHTML = ''; 
                    searchInput.focus(); 
                }, 500); // 500ms = ãÏÊ ÒãÇä ÇäíãíÔä ÏÑ CSS

            }, 3500); // 3500ms = ãÏÊ ÒãÇä äãÇíÔ ÞÈá ÇÒ ÔÑæÚ ãÍæ ÔÏä
        }
